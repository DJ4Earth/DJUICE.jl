...
# ╔═╡ 97fcae33-7c94-4768-8f9f-270fe9572620
using DJUICE
...
begin
	...
	md1.inversion.iscontrol = 1
	md1.inversion.vx_obs = zeros(md1.mesh.numberofvertices)
	md1.inversion.vy_obs = zeros(md1.mesh.numberofvertices)
	md1.inversion.independent = md1.friction.coefficient
	md1.inversion.independent_string = "FrictionCoefficient"
	
	with_logger(SimpleLogger(stdout, Logging.Error)) do
		solve(md1, :grad)
	end	
end;
...
As with the [previous tutorial](https://github.com/DJ4Earth/MPE24/blob/main/DJUICE_tutorials/Tutorial_DJUICE_1_basic.jl), you need to install the package [`DJUICE`](https://github.com/DJ4Earth/DJUICE.jl) if it is not already installed. 
...
or use `Pkg` to import `Project.toml` from the mini-tutorial

"""

# ╔═╡ a6f8b844-b9a2-4692-a747-c183deeade65
md"""
## Set up the model
...
# ╔═╡ 89e889f3-63f8-4f2a-bea3-4d8b736c5aa1
md"""
### Example Code to Load and Inspect the Model
"""

# ╔═╡ 771cc220-f9fc-4da3-9cca-f98d93465dff
plotmodel(md, md.friction.coefficient)

# ╔═╡ 0b7ecb9b-08f6-4e54-8f7c-5d71faa32c59
plotmodel(md, sqrt.(md.initialization.vx.^2+md.initialization.vy.^2))

# ╔═╡ 31624153-1463-4b7e-81f9-d33924b86e5a
md"""
...
# ╔═╡ cb691c03-16e1-43ec-bd97-7d7f6dd8ce6d
md"""
...
$$J = \frac{1}{2}\int_{\Omega}(u-u_{obs})^2+(v-v_{obs})^2\,\text{d}\Omega$$

For this tutorial, we set $u_{obs}$ and $v_{obs}$ to 0, simplifying the cost function to:

$$J = \frac{1}{2}\int_{\Omega}(u^2+v^2)\,\text{d}\Omega$$

Here, $J$ measures the velocity magnitudes of the ice flow. The gradient $\frac{\partial{J}}{\partial{C}}$ we compute is the sensitivity of the velocity to the basal friction coefficents $C$.
"""
...
plotmodel(md1, md1.results["StressbalanceSolution"]["Gradient"])

# ╔═╡ be7e4edc-5f33-4414-87f0-253e40928e82
plotmodel(md1, md1.friction.coefficient)
...
Use the instructions and example code above as a guide to write your own code. Compute the sensitivity and analyze the results. Furthermore, consider experimenting with different cost functions and independent variables to explore various aspects of the DJUICE model.
"""

# ╔═╡ 00000000-0000-0000-0000-000000000001
PLUTO_PROJECT_TOML_CONTENTS = """
[deps]
DJUICE = "1cb5fe60-6b70-416b-9050-039c23a62a10"
...
[[deps.DJUICE]]
deps = ["ColorSchemes", "Enzyme", "Flux", "GLMakie", "MAT", "Optimization", "OptimizationOptimJL", "Printf", "SparseArrays", "StatsBase", "Triangulate"]
git-tree-sha1 = "6611f28064f4c8fb5dacb8e765ddb7f08977bfdb"
...
git-tree-sha1 = "fe516248dcccd7285c0ac699eeaf32ae64d8e759"
...
git-tree-sha1 = "196a0a0ec2301de58a1e4cf2931211f6ae35f65f"
